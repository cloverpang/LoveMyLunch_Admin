...
var CloverJsonDataTemplateItem = function (opt) {
    var defaultOptions = {
        ...
        */
        ...
        Complete: undefined,
        /**
        * some url need to append the special query in url
        *
        * @type string
        */
        SpecialPara: '',
        ...
        */
        ...
        AppendTemplate: '',
        SaveCurrentSearchUrl: false,
        UserCurrentSearchUrl: false,
        /**
        * sometime we need to excute append javascript function
        *
        * @type string
        */
        AppendOtherJavascriptFunction: '',

        //分页显示相关
        DisplyPageSize: '#DisplyPageSize',
        DisplyTotalResult: '#DisplyTotalResult',
        DisplyCurrentPage: '#DisplyCurrentPage',
        DisplyTotalPage: '#DisplyTotalPage',
        Data: {}
    }, Loaded = 0;
        ...
        if (this.opts.LookupFields.length > 0) {
            ...
            for (i = 0; i < this.opts.LookupFields.length; i++) {
                ...
                try {
                    ...
                    if (this.opts.GetTextDesc == true) {
                        tempVal = this.getTextDesc(tempVal);
                    }
                    if (this.opts.HtmlDecode == true) {
                        tempVal = this.htmlDecode(tempVal);
                    }
    ...
    this.processData = function (self, data) {
        ...
        // set rowcount
        self.SetRowRount(data[self.opts.JsonTotalRowsParam]);
        //self.opts.TotalRows = data[self.opts.JsonTotalRowsParam];
        ...
        if (self.opts.PaginationCountcontrol !== "") {
            $(self.opts.PaginationCountcontrol).text(data.TotalRows);
        }
        ...
        // init counter
        var counter = 0;
        var binditems = self.getContextValue(self.opts.AjaxDataRoot, data);
        ...
        $.each(rowsArray, function (i, item) {
            ...
            if (item.CreateTime) {
                item.CreateTime = self.FormatDate(item.CreateTime);
            }
            if (item.UpdateTime) {
                item.UpdateTime = self.FormatDate(item.UpdateTime);
            }
            //if (item.productName) {
            //    item.productName = self.StringTruncat(item.productName, 60, '...');
            //}
            ...
            ////alert(x);
            ////current number
            //var j = self.opts.CurrentPage * self.opts.Norecs + i + 1;
            ////alert(j);
            //// render
            //self.ApplyToTable(x, j);

            if (i == 0 && self.opts.AppendTemplate != undefined && self.opts.AppendTemplate != "") {
                // apply append template
                var x = self.RenderAppendTemplate(item, rowClass);
                //current number
                var j = self.opts.CurrentPage * self.opts.Norecs + i + 1;
                //alert(j);
                // render
    ...
    this.GetData = function () {
        ...
        if (self.opts.Progress) {
            progressGuid = self.guidGenerator();
        }
        ...
        $.ajax({
            ...
            success: function (data, context) {
                ...
                //执行附加的js函数
                if (self.opts.AppendOtherJavascriptFunction != "") {
                    var str = self.opts.AppendOtherJavascriptFunction;
                    return eval(str);
                }
            ...
            complete: function () {
                if (self.opts.Progress) {
                    //alert(self.opts.Progress);
                    //$('#' + self.opts.Progress).hide();
                    $("." + progressGuid).hide(); 
                }
                // apply pagination
                if (self.opts.Paginate) {
                ...
                if (self.opts.ShowType == "table") {
                    $(self.opts.TblHeader).show();
                }

                if (self.opts.ShowType == "gridlist") {
                    $(self.opts.TblHeader).hide();
                }

                var numRows = $(self.opts.Tbl).find('tr').length + $(self.opts.Tbl).find('div').length + $(self.opts.Tbl).find('li').length + $(self.opts.Tbl).find('a').length;
    ...
    */
    ...
    this.RenderAppendTemplate = function (item, rowClass) {
        //alert("find the append template ing");
        //populate template                
        var x = $.tempest(this.opts.AppendTemplate, {
            item: item,
            rowBG: rowClass
        });
        //var x = this.opts.Template;
    ...
    this.ApplyToTable = function (x,j) {
        $(this.opts.Tbl).append(x);
        if (this.opts.ShowItemNumber == true) {
            var value = $(this.opts.Tbl).html().replace("{itemNumber}", j);
            //document.getElementById(this.opts.Tbl).innerHTML = " ";
            $(this.opts.Tbl).html(value);
        }
        $(this.opts.Tbl).fadeIn(this.opts.TableFadeInSpeed);
    };
    ...
    this.FormatDate = function (d) {
        ...
        if (part) {
            ...
            var mDate = $.datepicker.formatDate("yy-mm-dd", new Date(date.toString()));
            return mDate;
        }
        else {
            return d.toString();
    ...
    this.RefreshData = function () {
        ...
        this.ClearTable();
        //alert("old table cleared!");
        this.BuildURL();
        //alert("build url done");
        this.GetData();
        //alert("got data done");
    ...
    this.SetupSortControl = function () {
        ...
        if (self.opts.SortingControl != "undefined") {
            $(self.opts.SortingClassName).on("click", function () {
    ...
    */
    ...
    this.BuildURL = function () {
        ...
        if (this.opts.SaveCurrentSearchUrl == true) {
            cookieSearchUrl = GetCookie("currentSearchUrl");
            cookieServelUrl = GetCookie("currentSeverUrl");
        }

        if (this.opts.RememberRequestQuery != '' && this.opts.RememberRequestQuery != null && this.opts.ServerURL == this.opts.RememberServerUrl) {
            this.opts.Url = this.opts.RememberRequestQuery;
        }else{
            ...
            if (this.opts.SpecialPara != '') {
                this.opts.Url = this.opts.ServerURL + "?" + this.opts.SpecialPara
            }
            else {
                this.opts.Url = this.opts.ServerURL + "?SpecialPara=null"
            }

            if (this.opts.QueryAppend != '') {
                this.opts.Url = this.opts.Url + "&query=" + this.opts.Options + ":" + this.opts.QueryAppend
            }
            else {
                this.opts.Url = this.opts.Url + "&query=" + this.opts.Options
            }
			
			var page = parseInt(this.opts.CurrentPage) + 1;
            this.opts.Url = this.opts.Url
                + "&section=" + this.opts.Section
                + "&subSection=" + this.opts.Subsection
                + "&noOfItems=" + noOfItems
                + "&page=" + page
                + "&sortColumn=" + this.opts.SortField
                + "&sortType=" + this.opts.SortOrder
                + "&tag=" + this.opts.Tag
                + "&pageSize=" + this.opts.Pagesize
                + "&dataSource=" + this.opts.DataSource

            //alert("the url is : " + this.opts.Url);
            this.opts.Url += this.AddHackForIE();
        }

        if (this.opts.SaveCurrentSearchUrl == true) {
            AddCookie("currentSearchUrl", this.opts.Url.replace(/&amp;/g, "&"), 1);
            AddCookie("currentSeverUrl", this.opts.ServerURL, 1);
            AddCookie("currentViewPage", this.opts.CurrentPage, 1);
        }

        if (this.opts.SearchUrlKeep != '') {
            //alert(this.opts.Url);
            $(this.opts.SearchUrlKeep).text(this.opts.Url);
        }

        if (this.opts.ServerUrlKeep != '') {
            //alert(this.opts.Url);
            $(this.opts.ServerUrlKeep).text(this.opts.ServerURL);
        }

        if (this.opts.CurrentPageKeep != '') {
            //alert(this.opts.CurrentPage);
            $(this.opts.CurrentPageKeep).text(this.opts.CurrentPage);
        }

        //如果使用cookie的search url 则
        if (this.opts.UserCurrentSearchUrl == true && cookieSearchUrl != '' && this.opts.ServerURL == cookieServelUrl) {
            this.opts.Url = cookieSearchUrl;
        }
    };
    ...
    this.ApplyPagination = function (TotalRows, CurrentPage, PageSize) {
        ...
        //$(this.opts.HeaderResultID).text('总数:' + TotalRows);
        var numPages = Math.ceil(TotalRows / PageSize);
        //$(this.opts.HeaderResultID).text('第' + (CurrentPage + 1) + '页/共' + numPages + '页');
        ...
        //$(this.opts.DisplyFromToResultID).text('当前显示: ' + (CurrentPage * PageSize + 1) + ' - ' + to + ' 每页显示:' + PageSize + " | ");
        $(this.opts.DisplyFromToResultID).text('每页' + PageSize + '条/共' + TotalRows + '条');

        $(this.opts.DisplyPageSize).text(PageSize);
        $(this.opts.DisplyTotalResult).text(TotalRows);
        $(this.opts.DisplyCurrentPage).text(CurrentPage + 1);
        $(this.opts.DisplyTotalPage).text(numPages);
    ...
    // 文本的剪截
    this.getTextDesc = function (value) {
        var self = this;
        var result = self.htmlDecode(value);
        if (self.opts.FilterHtml) {
            result = self.filterHtml(result);
        }
    ...
    // 过滤 script, img, a标签
    this.filterHtml = function(value) {
        //return value.replace(/<(script)[\S\s]*?\1>|<\/?(a|img)[^>]*>/gi, "");
        return value.replace(/<[^>]+>/g, "");//去掉所有的html标记
    }
};                                                                                                                                                                                                                                                                                                                                                                                                       // end of class
